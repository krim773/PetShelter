...
export const homeShelter = async (req, res) => {
  try {
    //post por publicaciones...
    const posts = await Shelter.find();
    res.send(posts);
  } catch (error) {
...
export const updating = async (req, res) => {
  try {
    ...
    //consulta a trave del modelo Shelter
    // new true es para que muestr los datos actualizados
    const perfilActualizado = await Shelter.findByIdAndUpdate(
      req.params.id,
      req.body,
      {
        new: true,
...
export const getProfileID = async (req, res) => {
  try {
    //parecido al delete > tras consultar a trvez del modelo shelter se buscara el perfil por id...
    const perfilUsuario = await Shelter.findById(req.params.id);
...
export const deleteProfile = async (req, res) => {
  try {
    //consulta a travez del modelo shelter
    //el id va a venir desde req.params.id
    const profileRemove = await Shelter.findByIdAndDelete(req.params.id);
